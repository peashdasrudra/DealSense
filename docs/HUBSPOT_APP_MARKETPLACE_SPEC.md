...
| :--- | :--- | :--- | :---: |
| **OAuth 2.0 & Scopes** | Least-privilege principle; tokens must refresh automatically; CSRF `state` validation; frictionless test installs. | Stateless HMAC-SHA256 CSRF protection + direct-install fallback for Developer Portal reviewers. In-flight code deduplication against React 18 double mounts. Harmonized CRM v3 scopes (`deals`, `contacts`, `companies`, `schemas`, `notes`, `owners`, `timeline`). Proactive token refresh with 5-min safety threshold and Fernet encryption at rest. | **COMPLIANT** |
