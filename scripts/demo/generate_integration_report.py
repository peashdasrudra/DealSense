#!/usr/bin/env python3
"""DealSense — Integration Proof Report Generator.

Generates a beautiful, shareable HTML/Markdown integration proof report
that can be shown during live interview calls or sent beforehand.

Usage:
    python scripts/demo/generate_integration_report.py
"""
...
def generate_markdown_report(api_results: dict | None = None) -> str:
    """Generate a comprehensive Markdown integration proof report."""

    timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")

    report = f"""# 🔬 DealSense — Live Integration Proof Report
...
*© {datetime.now().year} AiXpert Labs / Peash Das Rudra*
"""
    return report


def main() -> None:
    """Generate and save the integration proof report."""
    print("📊 Generating DealSense Integration Proof Report...")
    ...
    md_path = report_dir / "INTEGRATION_PROOF_REPORT.md"
    md_path.write_text(report_md, encoding="utf-8")
    print(f"  ✅ Markdown report: {md_path}")

    # Also save timestamped version
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    timestamped_path = report_dir / f"integration_proof_{timestamp}.md"
    timestamped_path.write_text(report_md, encoding="utf-8")
    print(f"  ✅ Timestamped copy: {timestamped_path}")

    print("\n  📄 Share this report with interviewers or display on screen.")
