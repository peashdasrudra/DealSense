...
// 1. Rename Component and remove BookTriageModal
...
content = content.replace(/import \{ BookTriageModal \} from "\.\/BookTriageModal";/g, '');

// 2. Add Upgrade Modal state instead of Booking State
...
content = content.replace('const [bookingTier, setBookingTier] = useState<"audit-99" | "retainer">("audit-99");', '');
content = content.replace(/const openBooking = \(([^)]+)\) => \{[^}]+\};/g, '');

// 3. Replace the entire bottom Action Box (Agency Services) with the Enterprise Upgrade CTA
const actionBoxRegex = /\{\/\* \-\- BOTTOM CALL TO ACTION \-\- \*\/\}.*?(?=\{?\/\* Booking Sub-Modal)/s;
const upgradeBoxHTML = `
...
`;
content = content.replace(actionBoxRegex, upgradeBoxHTML);

// Remove BookTriageModal component rendering completely just in case
content = content.replace(/<BookTriageModal[\s\S]*?\/>/g, '');

// 4. Run Light-Mode Replacements
const replacements = {
